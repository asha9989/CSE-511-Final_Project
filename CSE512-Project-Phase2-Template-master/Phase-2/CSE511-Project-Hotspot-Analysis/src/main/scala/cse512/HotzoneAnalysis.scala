...
object HotzoneAnalysis {
  ...
  def runHotZoneAnalysis(spark: SparkSession, pointPath: String, rectanglePath: String): DataFrame = {
    ...
    val rect_join = spark.sql("select rectangle, count(point) from joinResult group by rectangle order by rectangle")
    rect_join.createOrReplaceTempView("rectJoin")
    return rect_join.coalesce(1) // YOU NEED TO CHANGE THIS PART
  }
